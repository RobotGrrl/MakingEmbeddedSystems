...
#include "../../../Drivers/BSP/STM32L496G-Discovery/stm32l496g_discovery_audio.h"
...
//	}
  ...
  while (1)
  {
  	...
  	/*
  	// WTF
  	if(HAL_GetTick()-last_refresh >= 100) { // this is in ms

			// BSP_LCD_Clear 'glitches' the display each loop
			// BSP_LCD_SetBackColor doesn't clear the display
			// try draw a rectangle?

			BSP_LCD_SetTextColor( LCD_COLOR_WHITE );
			BSP_LCD_FillRect(0, 120-20, 240, 20*2);
			...
			BSP_LCD_SetTextColor( LCD_COLOR_DARKMAGENTA );
			BSP_LCD_FillCircle(xpos, ypos, 20);
